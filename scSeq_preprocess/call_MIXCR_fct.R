...
#'
#'
#'######## FUNCTION ADJUSTED BY LENA ERLACH, 4TH JULY 2024
#' only useable for Platypus v3 and MAC/Linux system 
...
VDJ_call_MIXCR <- function(VDJ,
  ...
  # } else 
    ...
    if (platypus.version == "v3" & operating.system %in% c("Darwin", "Linux")) {
    ...
    seqinr::write.fasta(sequences = as.list(temp.seq.hc_unlist),names = temp.name.hc_unlist,file.out = "temphc.fasta")
    system(paste(mixcr.directory," align -p generic-bcr-amplicon --dna --rigid-left-alignment-boundary --rigid-right-alignment-boundary --force-overwrite -OsaveOriginalReads=true -s ", species," temphc.fasta tempmixcrhc.out.vdjca",sep=""))
    # print("done1")
    system(paste(mixcr.directory," exportAlignments --impute-germline-on-export -allAAFeatures FR1Begin FR4End -allNFeatures FR1Begin FR4End -allAAMutations FR1Begin FR4End -allNMutations FR1Begin FR4End -cloneId --force-overwrite -descrsR1 -vAlignment -dAlignment -jAlignment -aaMutations VRegion -aaMutations JRegion -nMutations VRegion -nMutations JRegion tempmixcrhc.out.vdjca tempmixcrhc.out.tsv",sep=""))
                 
                 
                 
                 #--preset full -descrsR1 -vAlignment -dAlignment -jAlignment -aaMutations VRegion -aaMutations JRegion -nMutations VRegion -nMutations JRegion tempmixcrhc.out.vdjca tempmixcrhc.out.tsv",sep=""))
    # print("exported1")
    
    seqinr::write.fasta(sequences = as.list(temp.seq.lc_unlist),names = temp.name.lc_unlist,file.out = "templc.fasta")
    system(paste(mixcr.directory," align -p generic-bcr-amplicon --dna --rigid-left-alignment-boundary --rigid-right-alignment-boundary --force-overwrite -OsaveOriginalReads=true -s ", species," templc.fasta tempmixcrlc.out.vdjca",sep=""))
    # print("done2")
    system(paste(mixcr.directory," exportAlignments --impute-germline-on-export -allAAFeatures FR1Begin FR4End -allNFeatures FR1Begin FR4End -allAAMutations FR1Begin FR4End -allNMutations FR1Begin FR4End -cloneId --force-overwrite -descrsR1 -vAlignment -dAlignment -jAlignment -aaMutations VRegion -aaMutations JRegion -nMutations VRegion -nMutations JRegion tempmixcrlc.out.vdjca tempmixcrlc.out.tsv",sep=""))
    # print("exported2")
    
    temp.mixcr.hc <- utils::read.table(file ="tempmixcrhc.out.tsv", sep="\t", header = T, stringsAsFactors=FALSE, fill=TRUE)
    temp.mixcr.lc <- utils::read.table(file ="tempmixcrlc.out.tsv", sep="\t", header = T, stringsAsFactors=FALSE, fill=TRUE)
    
    system("rm temphc.fasta")
    system("rm templc.fasta")
    system("rm tempmixcrhc.out.vdjca")
    system("rm tempmixcrlc.out.vdjca")
    system("rm tempmixcrhc.out.tsv")
    system("rm tempmixcrlc.out.tsv")
    ...
    if(simplify == F){
      ...
      #add selected columns
      to_merge_hc <- temp.mixcr.hc[,c("nSeqFR1","nSeqCDR1", "nSeqFR2","nSeqCDR2", "nSeqFR3","nSeqCDR3", "nSeqFR4", "aaSeqFR1","aaSeqCDR1", "aaSeqFR2","aaSeqCDR2", "aaSeqFR3","aaSeqCDR3", "aaSeqFR4",
                                      "nSeqImputedFR1", "nSeqImputedCDR1", "nSeqImputedFR2","nSeqImputedCDR2", "nSeqImputedFR3","nSeqImputedCDR3", "nSeqImputedFR4",
                                      "aaSeqImputedFR1","aaSeqImputedCDR1", "aaSeqImputedFR2","aaSeqImputedCDR2", "aaSeqImputedFR3","aaSeqImputedCDR3", "aaSeqImputedFR4")]
      #add SHM measures
      to_merge_hc$SHM <- stringr::str_count(temp.mixcr.hc$bestVAlignment,"S") + stringr::str_count(temp.mixcr.hc$bestJAlignment,"S") + stringr::str_count(temp.mixcr.hc$bestDAlignment,"S")
      #append prefix to names
      names(to_merge_hc) <- paste0("VDJ_",names(to_merge_hc))
  ...
  } else {print('This function only runs with platypus.version == "v2" and operating.system "Darwin" & "Linux"')}
