...
integrate_datasets <- function(gex_list) {
  ...
  immune.anchors <- FindIntegrationAnchors(object.list = gex_list, anchor.features = features)
  ...
  return(immune_combined) 
}

filter_tcells <- function(GEX_ds, clusters, umap_dims = 30, neigh_dims = 10, clust_res = 0.5) {
  immune.combined_tex <- subset(GEX_ds, subset = seurat_clusters %in% clusters)
  
  immune.combined_tex <- immune.combined_tex %>%
    ScaleData() %>%
    RunPCA() %>%
    RunUMAP(dims = 1:umap_dims) %>%
    
    return(immune.combined_tex)
}

cdr_comb_fct <- function(VDJ_mixcr) {
...
vgm_out_path <-  paste0(getwd(), "/Analysis_test/data/")
...
# saveRDS(VDJ_GEX_matrix[["OVA"]], paste0(vgm_out_path, "raw/raw_VDJ_GEX_object_OVA_integrated_harmony.rds"))
# saveRDS(VDJ_GEX_matrix[["RBD"]], paste0(vgm_out_path, "raw/raw_VDJ_GEX_object_RBD_integrated_harmony.rds"))
...
##### Preprocess OVA file; 
for (ds in c("OVA", "RBD")) {
  VDJ_GEX_matrix[[ds]] <- data_prep(VDJ_GEX_matrix[[ds]], rename_group_id_OVA=FALSE, filter_1HCLC = FALSE)
  VDJ_GEX_matrix[[ds]][["GEX"]] <- OVA_RBD_renaming(GEX = VDJ_GEX_matrix[[ds]][["GEX"]])
}
...
saveRDS(VDJ_GEX_matrix[["OVA"]], paste0(vgm_out_path, "raw/procVDJ_GEX_OVA_intermediate.rds"))
saveRDS(VDJ_GEX_matrix[["RBD"]], paste0(vgm_out_path, "raw/procVDJ_GEX_RBD_intermediate.rds"))
...
### Preprocess VDJ and VJ sequences
for (dataset in c("OVA", "RBD")) {
  
  VDJ_GEX_matrix[[dataset]][["VDJ"]] <- cdr_comb_fct(VDJ_GEX_matrix[[dataset]][["VDJ"]])
...
# VDJ_GEX_matrix <- list()
# VDJ_GEX_matrix[["OVA"]] <- readRDS(paste0(vgm_out_path, "raw/procVDJ_GEX_OVA_intermediate.rds"))
# VDJ_GEX_matrix[["RBD"]] <- readRDS(paste0(vgm_out_path, "raw/procVDJ_GEX_RBD_intermediate.rds"))



### Combine the VDJ_matrices for OVA and RBD
VDJ_GEX_matrix[["INT"]][["VDJ"]] <- rbind(VDJ_GEX_matrix[["OVA"]][["VDJ"]], VDJ_GEX_matrix[["RBD"]][["VDJ"]])


####### Run harmony

# Combine to list
gex_list <- list(VDJ_GEX_matrix[["OVA"]][["GEX"]], VDJ_GEX_matrix[["RBD"]][["GEX"]] )

immune_combined <- integrate_datasets(gex_list)

VDJ_GEX_matrix[["INT"]][["GEX"]] <- immune_combined



######## SAVE INTERMEDIATE FILE!! 
saveRDS(VDJ_GEX_matrix[["INT"]], paste0(vgm_out_path, "raw/proc_VDJ_GEX_OVA_RBD_integrated_harmony.rds"))
...
VDJ_GEX_matrix[["OVA"]] <- readRDS(paste0(vgm_out_path, "raw/procVDJ_GEX_OVA_intermediate.rds"))
VDJ_GEX_matrix[["RBD"]] <- readRDS(paste0(vgm_out_path, "raw/procVDJ_GEX_RBD_intermediate.rds"))
VDJ_GEX_matrix[["INT"]] <- readRDS(paste0(vgm_out_path, "raw/proc_VDJ_GEX_OVA_RBD_integrated_harmony.rds"))
...
GEX <- filter_tcells (VDJ_GEX_matrix[["OVA"]][["GEX"]], clusters=c(0:9,11, 12), umap_dims = 30)
...
VDJ_mat <- subset(VDJ_GEX_matrix[["RBD"]][["VDJ"]], subset= seurat_clusters %in% c(0:6,8,9,11,12))

GEX <- filter_tcells (VDJ_GEX_matrix[["RBD"]][["GEX"]], clusters=c(0:6,8,9,11,12), umap_dims = 30)
...
###### INTEGRATED
DimPlot(VDJ_GEX_matrix[["INT"]][["GEX"]], reduction = "umap", label = TRUE, pt.size = 0.7) + 
  FeaturePlot(VDJ_GEX_matrix[["INT"]][["GEX"]], features=c("rna_CD3E"))

VDJ_GEX_matrix[["INT"]][["GEX"]] <- filter_tcells(VDJ_GEX_matrix[["INT"]][["GEX"]], clusters= c(0:10,13,14), umap_dims = 30)
...
VDJ_mat <- subset(VDJ_mat, subset = orig_barcode %in% bc_rm, invert=TRUE)
...
GEX_integr <- subset(VDJ_GEX_matrix[["INT"]][["GEX"]], subset = orig_barcode %in% bc_rm, invert=TRUE)
## remove from VDJ_mat
...
DimPlot(VDJ_GEX_matrix[["INT"]][["GEX"]]) + ggtitle("INT") + 
  DimPlot(VDJ_GEX_matrix[["OVA"]][["GEX"]]) + ggtitle("OVA") +
  DimPlot(VDJ_GEX_matrix[["RBD"]][["GEX"]]) + ggtitle("RBD")
...
saveRDS(VDJ_GEX_matrix[["OVA"]], paste0(vgm_out_path, "VDJ_GEX_object_OVA_harmony_overlapping_clones_rm.rds"))
...
saveRDS(VDJ_GEX_matrix[["RBD"]], paste0(vgm_out_path, "VDJ_GEX_object_RBD_harmony_overlapping_clones_rm.rds"))
