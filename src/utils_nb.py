...
def load_pickle_embeddings(names, inputPath, file_suffix="", verbose=False):
    ...
    for file_name in names:
        try:
            with open(
                os.path.join(inputPath, f"{file_name}{file_suffix}.p"), "rb"
            ) as file:
...
def load_pickle_embeddings_VH_VL(
    names, inputPath, embedding_type="fixed", file_suffix="", verbose=False
):
    loaded_data = []
    ...
    for file_name in range(len(names)):
        try:
            ...
            with open(os.path.join(inputPath, f"{names_VH[file_name]}"), "rb") as file:
                ...
                if verbose is True:
                    print(f"Loaded {names_VH[file_name]}")
            ...
            with open(os.path.join(inputPath, f"{names_VL[file_name]}"), "rb") as file:
                ...
                if verbose is True:
                    print(f"Loaded {names_VL[file_name]}")
...
def compute_distance_matrix(sequences, k):
    ...
    vectors = [
        freqs_to_vector(kmer_frequencies(seq, k), all_kmers) for seq in sequences
    ]
...
def generate_ESM_embedding(
    ...
    if VH_only is True:
        ...
        for index, row in tqdm(df.iterrows(), total=df.shape[0]):
            ...
            for model_typ in ["esm2"]:
                ...
                out_path_aug = os.path.join(
                    out_folder, model_typ + "_aug_" + str(k) + "_VH/"
                )
                ...
                if augment is True:
                    ...
                    kmut_matr = prot_embed.create_kmut_matrix(
                        num_muts=k, embed_type=model_typ
                    )
                    cdr_embed = prot_embed.create_cdr_embedding(
                        kmut_matr, sep=False, mask=True
                    )
    ...
    # generate VH_VL embeddings
        ...
        if save_PLM_aug:
            out_path_aug = os.path.join(
                out_folder, model_typ + "_aug_" + str(k) + "_VH_VL"
            )
            if not os.path.isdir(out_path_aug):
                os.makedirs(out_path_aug)
...
def generate_AbMAP_embedding(
    ...
    if not VH_only:
        out_path = os.path.join(
            out_folder, f"{model_typ}_k_{str(k)}_VH_VL_{embed_type}"
        )
    ...
    abmap_H = abmap.load_abmap(
        pretrained_path=pretrained_path_H_ls[0], device=cuda_dev_num, plm_name="esm2"
    )

    if not VH_only:  # Add a colon after the condition
        abmap_L = abmap.load_abmap(
            pretrained_path=pretrained_path_H_ls[0],
            device=cuda_dev_num,
            plm_name="esm2",
        )
...
def extract_cdr_embedding(
    ...
    if paired_chains:
        print("paired")
        assert len(seqs_H) == len(
            seqs_L
        ), "List of heavy and light chain seqs not equal"

        for seq_H, seq_L, embs in tqdm(
            zip(seqs_H, seqs_L, embeddings), total=len(seqs_H)
        ):
            #     print(seq_H)
            #     print(seq_L)
            embedding_H = embs[0]
...
def pca_plot(
    fl_embeddings: np.array,
    seq_df: pd.DataFrame,
    camsol_raw: pd.DataFrame = None,
    sample_ids: list = None,
    group_ids: list = None,
    n_comps: int = 2,
    n_comps_plot: tuple = (0, 1),
    model_name: str = "",
    plot_subtype="all",
):
    """
    ...
    if plot_subtype == "camsol" or plot_subtype == "all":
        # plot with CamSol
        points = axs[0, 0].scatter(
            PCs[:, n_comps_plot[0]],
            PCs[:, n_comps_plot[1]],
            c=camsol_raw["protein variant score"],
            s=8,
            cmap="plasma",
        )
        ...
        axs[0, 1].scatter(
            PCs[camsol_raw["protein variant score"] > 0][:, 0],
            PCs[camsol_raw["protein variant score"] > 0][:, 1],
            s=8,
            label="> 0",
        )
        axs[0, 1].scatter(
    ...
    if plot_subtype == "groups" or plot_subtype == "all":
        axs[1, 0].scatter(
            PCs[seq_df["group_id"] == group_ids[0]][:, n_comps_plot[0]],
            PCs[seq_df["group_id"] == group_ids[0]][:, n_comps_plot[1]],
            c="orange",
            s=15,
            alpha=0.2,
            label="non-spec",
        ...
        axs[1, 0].scatter(
            PCs[seq_df["group_id"] == group_ids[1]][:, n_comps_plot[0]],
            PCs[seq_df["group_id"] == group_ids[1]][:, n_comps_plot[1]],
            c="purple",
            alpha=0.5,
            s=15,
            label="spec",
        ...
        axs[1, 1].scatter(
            PCs[seq_df["sample_id"] == samples_ids[0]][:, n_comps_plot[0]],
            PCs[seq_df["sample_id"] == samples_ids[0]][:, n_comps_plot[1]],
            s=8,
            label="s2",
        )
        axs[1, 1].scatter(
            PCs[seq_df["sample_id"] == samples_ids[1]][:, n_comps_plot[0]],
            PCs[seq_df["sample_id"] == samples_ids[1]][:, n_comps_plot[1]],
            s=8,
            label="s4",
        )
        axs[1, 1].scatter(
            PCs[seq_df["sample_id"] == samples_ids[2]][:, n_comps_plot[0]],
            PCs[seq_df["sample_id"] == samples_ids[2]][:, n_comps_plot[1]],
            s=8,
            label="s1",
        )
        axs[1, 1].scatter(
            PCs[seq_df["sample_id"] == samples_ids[3]][:, n_comps_plot[0]],
            PCs[seq_df["sample_id"] == samples_ids[3]][:, n_comps_plot[1]],
            s=8,
            label="s3",
        )
...
def run_UMAP(
    ...
    manifold = umap.UMAP(
        random_state=42, n_neighbors=n_neighbors, min_dist=min_dist
    ).fit(X, y)
...
def UMAP_emb_plots(
    ...
    axs[0, 0].scatter(
        X_reduced[:, 0][seq_df["sample_id"] == sample_ids[0]],
        X_reduced[:, 1][seq_df["sample_id"] == sample_ids[0]],
        c="lightgrey",
        s=20,
        label="s4",
        alpha=0.2,
    ...
    axs[0, 0].scatter(
        X_reduced[:, 0][seq_df["sample_id"] == sample_ids[1]],
        X_reduced[:, 1][seq_df["sample_id"] == sample_ids[1]],
        c="lightgrey",
        s=20,
        label="s2",
        alpha=0.2,
    ...
    axs[0, 0].scatter(
        X_reduced[:, 0][seq_df["sample_id"] == sample_ids[2]],
        X_reduced[:, 1][seq_df["sample_id"] == sample_ids[2]],
        c="y",
        s=20,
        label="s1",
        alpha=0.5,
    ...
    axs[0, 0].scatter(
        X_reduced[:, 0][seq_df["sample_id"] == sample_ids[3]],
        X_reduced[:, 1][seq_df["sample_id"] == sample_ids[3]],
        c="r",
        s=20,
        label="s3",
        alpha=0.5,
    ...
    axs[0, 1].scatter(
        X_reduced[:, 0][seq_df["group_id"] == group_ids[0]],
        X_reduced[:, 1][seq_df["group_id"] == group_ids[0]],
        c="orange",
        s=20,
        label="nonspec",
        alpha=0.2,
    ...
    axs[0, 1].scatter(
        X_reduced[:, 0][seq_df["group_id"] == group_ids[1]],
        X_reduced[:, 1][seq_df["group_id"] == group_ids[1]],
        c="purple",
        s=20,
        label="spec",
        alpha=0.5,
    ...
    axs[1, 1].scatter(
        X_reduced[camsol_raw["protein variant score"] > 0][:, 0],
        X_reduced[camsol_raw["protein variant score"] > 0][:, 1],
        s=10,
        label="> 0",
    )
    axs[1, 1].scatter(
...
def reg_param_tuning(
    ...
    # setup k fold
    kf = KFold(
        n_splits=k, shuffle=True, random_state=1
    )  # Define the n_split = number of folds
...
### Correlation plot
def regression_correlation_plot(
    y_test: np.array,
    y_pred: np.array,
    ...
    leg = plt.legend(labels=[l0], handlelength=0, handletextpad=0, loc=4)
...
def tune_predict_KRR(X_train, X_test, y_train, y_test, k):
    ...
    reg = KernelRidge()
    pipeline = Pipeline([("scaler", scaler), ("regressor", reg)])
    metrics = {"neg_MSE": "neg_mean_squared_error", "r2": "r2"}
    # define parameter grid
    param_grid = {
        "regressor__alpha": [0.1, 1.0, 10.0, 50, 100],
    ...
    parma_tuned_GS = reg_param_tuning(
        X_train, y_train, pipeline, param_grid, metrics, k=k, n_jobs=-1, timed=True
    )
...
def tune_predict_GP(X_train, X_test, y_train, y_test, k):
    ...
    # define parameter grid
    kernel_list = [RBF(l) for l in np.logspace(-1, 1, 3)] + [
        Matern(l) for l in np.logspace(-1, 1, 3)
    ]  # noqa: E741  # noqa: E741
    param_grid = {
        "regressor__kernel": kernel_list,
...
def grouped_train_test_split(X, y, test_size, group_thresh, verbose=1):
    ...
    train_idx, test_idx = next(
        GroupShuffleSplit(n_splits=1, test_size=test_size, random_state=123).split(
            X, groups=clusters
        )
    )
...
def regression_correlation_plots(
    y_test: np.array,
    y_preds: list,
    title: str = "Correlation plot",
    methods=["KRR", "GP"],
    ...
    for i, (method, y_pred) in enumerate(zip(methods, y_preds)):
        ...
        axs[i].scatter(y_test, y_pred, s=8)
        ...
        axs[i].legend(
            labels=[l0], handlelength=0, handletextpad=0, loc=4, markerscale=0
        )
...
# split train test set by clustering similar sequences
def return_grouped_train_test_split(
    X, y, test_size, group_thresh, n_splits=5, verbose=1
):
    sim = 1 - group_thresh
...
def evaluate_clf(X, y, test_size, group_thresh, n_splits=5):
    gss, clusters = return_grouped_train_test_split(
        X, y, test_size, group_thresh, n_splits=5, verbose=1
    )
...
def cluster_map(
    ...
    def emb_scatter_plots(
        distance_matrix1,
        distance_matrix2,
        seq_df,
        x_lab="embedding dist1",
        y_lab="embedding dist2",
    ):
        # prepare the data
        df1 = np.tril(distance_matrix1, k=0)
        df1 = df1[df1 != 0.0].ravel()
        df2 = np.tril(distance_matrix2, k=0)
        df2 = df2[df2 != 0.0].ravel()
        ...
        # plot with density
        axs[0].scatter(df1, df2, c=z, s=0.5, alpha=1)
        ...
        for i, j in zip([2, 1], ["nonspec", "spec"]):
            ...
            axs[1].scatter(df1, df2, s=0.5, alpha=1, label=j)
