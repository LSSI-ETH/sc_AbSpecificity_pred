...
def return_grouped_train_test_split(
    ...
    if cluster_method in [
        ...
        if cluster_method == "levenshtein_sequence_based":
            ...
            if verbose > 0:
                print("Calculate clusters from precomputed distance matrix")

            # Assuming distances is your distance matrix
            linked = linkage(
                squareform(distance_matrix), "single"
            )  # You can also use 'complete', 'average', etc.
            ...
            if verbose > 0:
                print("Calculate clusters from embeddings with DBSCAN")
            ...
            if verbose > 0:
                print("Calculate clusters from embeddings with hierarchical clustering")
            # Compute the pairwise distance matrix
            distance_matrix = pairwise_distances(X)
...
def train_test_split_idx(
    ...
    if cluster_method in [
        ...
        if verbose > 0:
            print("Return random splits")

        gss = StratifiedShuffleSplit(
            test_size=1 / n_splits, n_splits=n_splits, random_state=123
        )
        clusters = None
...
class Specificity_classificiation_model_evaluation:
    ...
    def __init__(
        ...
        X: np.ndarray,
        y: np.ndarray,
        metrics: dict = {"AUC": "roc_auc", "f1": "f1", "recall": "recall"},
    ...
    def run_clf(
        self, model_name, param_grid, clf, X_train, X_test, y_train, y_test, verbose=1
    ):
        """
    ...
    def evaluate_clf(self, X, y, train_id_ls, test_id_ls, log=None, RF=None, verbose=1):
        metric_dict_Log = {}
        metrics_ls = [
            "accuracy",
            "MCC",
...
def run_clf_on_splits(
    ...
    for splits, cluster_m in zip(
        train_test_splits, ["levenshtein_sequence_based", "random_split"]
    ):
        if log is not None:
        ...
        metric_dict_Log, metric_dict_SVC, test_idx = clf.evaluate_clf(
            X,
            y,
            train_id_ls=splits[0],
            test_id_ls=splits[1],
            RF=RF,
            log=None,
            verbose=1,
        ...
        if log is not None:
            log.info(
                f"\n{model1} - MCC: ",
                np.mean(metric_dict_Log["MCC"]),
                np.std(metric_dict_Log["MCC"]),
            )
        else:
            print(
                f"\n{model1} - MCC: ",
        ...
        if log is not None:
            log.info(
                f"\n{model2} - MCC: ",
                np.mean(metric_dict_SVC["MCC"]),
                np.std(metric_dict_SVC["MCC"]),
            )
        else:
            print(
                f"\n{model2} - MCC: ",
