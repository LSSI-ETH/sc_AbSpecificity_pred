...
class LoadEmbeddings_VH_VL:
    def __init__(
        ...
        assert seq_col in [
            "VDJ_VJ_aaSeq",
            "VDJ_aaSeq",
        ], 'seq_col must be either "VDJ_VJ_aaSeq" or "VDJ_aaSeq"'
        ...
        self.emb_inputPath_ESM = os.path.join(
            self.ROOT_DIR, config[config_dir]["ESM2_var"]
        )

        # CDR extracted embeddings
        self.emb_inputPath_ESM_cdrs = os.path.join(
        ...
        dist_matrix_inputPath = os.path.join(
            self.ROOT_DIR, config[config_dir]["DISTANCE_MATRIX"]
        )
    ...
    def load_embeddings(self, embedding_type="all", VH_emb_fname_suff="", verbose=True):
        ...
        if self.seq_col == "VDJ_VJ_aaSeq":
            if verbose is True:
                print("Load embeddings for VH_VL sequences")

            # iterate through all embeddings and load them
            if embedding_type == "all" or embedding_type == "esm":
                ### ESM2 - VH_VL
                self.emb_ESM_var = utils.load_pickle_embeddings_VH_VL(
                    self.names, self.emb_inputPath_ESM, embedding_type="var"
                )
                self.emb_ESM = utils.mean_over_HL(
                    self.emb_ESM_var
                )  # average embeddings over across seq_len & VH+VL
                if verbose is True:
                    print("ESM - VH_VL embeddings loaded")
            ...
            if embedding_type == "all" or embedding_type == "antiberty":
                ...
                self.emb_antiberty_var = utils.load_pickle_embeddings_VH_VL(
                    self.names,
                    self.emb_inputPath_antiberty,
                    file_suffix="",
                    embedding_type="var",
                )
                self.emb_antiberty = utils.mean_over_HL(self.emb_antiberty_var)
                if verbose is True:
                    print("Antiberty - VH_VL embeddings loaded")

        elif self.seq_col == "VDJ_aaSeq":
            if verbose is True:
                print("Load embeddings for VH sequences")
            ...
            if embedding_type == "all" or embedding_type == "esm":
                ### ESM2 - VH
                self.emb_ESM_var = utils.load_pickle_embeddings(
                    self.names, self.emb_inputPath_ESM
                )
                self.emb_ESM = np.array(
                    [emb.mean(0) for emb in self.emb_ESM_var]
            ...
            if embedding_type == "all" or embedding_type == "antiberty":
                ### Antiberty - VH
                self.emb_antiberty_var = utils.load_pickle_embeddings(
                    self.names, self.emb_inputPath_antiberty, file_suffix="_H"
                )
                self.emb_antiberty = np.array(
                    [emb.mean(0) for emb in self.emb_antiberty_var]
                )
