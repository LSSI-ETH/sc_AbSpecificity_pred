...
parser = argparse.ArgumentParser(
    description="Run classification models to predict Binding Specificity values from gene expression"
)
parser.add_argument(
...
def run():
    ...
    log = utils.Logger(
        __name__, log_file=f"BCR_GEX_{today}_{c_type}_app.log"
    ).get_logger()
    ...
    for dataset in d_list:
        ...
        try:
            ...
            distance_matrix = utils.calc_norm_levens_dist(
                feature_df[ab_chain], verbose=0
            )
        ...
        try:
            ...
            file_path = os.path.join(
                out_path,
                f"{today}_{dataset}_{SIM_SPLIT}_GEX_Spec_classification_CV_results.csv",
            )
