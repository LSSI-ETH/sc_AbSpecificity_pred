...
parser = argparse.ArgumentParser(
    description="Run classification models to predict Binding Specificity for from sequence embeddings"
)
parser.add_argument(
...
def encode_data(seq_list, names):
    ...
    aa_seqs = [seq + (max_seq_len - len(seq)) * "-" for i, seq in enumerate(aa_seqs)]
...
def run():
    ...
    try:
        ...
        train_ls, test_ls, clusters = CLF.train_test_split_idx(
            X,
            y,
    ...
    try:
        ...
        file_path = os.path.join(
            out_path, f"{today}_{c_type}_1hot_OVA_Spec_classification_CV_results.csv"
        )
